...
Regular Expression Based Matchers
---------------------------------

Because some architectures require a finely grained control of your resources,
it's also possible to assign a regular expression to a segment. Let's see how
the blog example can take advantage of this:

.. code-block:: python

    class Root(resource.Resource):

        @resource.child('{year:[0-9]{4}}/{month:[01][0-9]}')
        def blog_month_entries(self, request, segments, **kw):
            return BlogList(**kw), segments

        @resource.child('{year:[0-9]{4}}/{month:[01][0-9]}/{entryid:[0-9]+}')
        def blog(self, request, segments, **kw):
            return BlogPost(**kw), segments

        @resource.child('feeds/{type:atom|rss|rss2}.xml')
        def feeds(self, request, segments, **kw):
            return BlogFeed(**kw), segments

        @resource.child()
        def archives(self, request, segments):
            return BlogArchives(), segments

Because we know what kind of data the various fields will contain, it's then
possible to share a common space between different kind of resources. There
